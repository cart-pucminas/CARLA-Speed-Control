...
from Detections.PlateDetector import PlateDetector
from Detections.PlateClassification import PlateClassification
...
class BasicAgent(object):
    ...
    def __init__(self, vehicle, args, target_speed=20, opt_dict={}, map_inst=None, grp_inst=None):
        """
        ...
        self._max_brake = 0.5
        self._offset = 0
        self._plate_detector = PlateDetector(args.detector_model_path)
        self._plate_classify = PlateClassification(args.classifier_model_path)
    ...
    def plate_detector(self, img):
        """Detects the plate in the image"""
        results =  self._plate_detector.detect(img)
        return results
    
    def plate_classify(self, img):
        """Classifies the plate in the image"""
        results = self._plate_classify.detect(img)
        return results
